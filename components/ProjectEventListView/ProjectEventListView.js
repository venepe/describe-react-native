...
class ProjectEventListView extends Component {
  ...
  _getInitialState() {
    ...
    return {
      hasNextPage: false,
      dataSource: ds.cloneWithRows(this.props.project.events.edges)
    };
  }
  ...
  _getUpdatedState(project) {
    ...
    return {
      dataSource: ds.cloneWithRows(project.events.edges)
    };
  }
  ...
  _renderRow(rowData, sectionID, rowID)  {
    ...
    let previous = (index > 0) ? this.props.project.events.edges[index - 1].node.text : current;
